...
def water_levels_xml2(manual, pressure):
    if not pressure:
        ...
        for k, record in sorted(records, key=lambda r: r[1][4].date() if r[0] == 'pressure' else r[1][1]):
            if k == 'pressure':
...
def make_continuous_water_level(root, r):
    ...
    for attr, val in (('DepthFromLandSurfaceData', '{:0.2f}'.format(r[columns.index('DepthToWaterBGS')])),
...
def make_water_level(root, r):
    ...
    # m = datetime.strptime(m, '%Y-%m-%d')
    for attr, val in (('DepthFromLandSurfaceData', '{:0.2f}'.format(r[2])),
                      ('WaterLevelUnits', r[3]),
                      ('MeasuringMethod', r[4]),
                      ('MeasurementMonth', m.month),
                      ('MeasurementDay', m.day),
